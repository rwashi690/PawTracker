...
router.get('/', ensureAuthenticated, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: 'Failed to get pets', details: error instanceof Error ? error.message : 'Unknown error' });
  }
});

// Add a new pet
router.post('/', ensureAuthenticated, upload.single('image'), async (req: Request, res: Response): Promise<void> => {
  try {
...
router.put('/:id/image', ensureAuthenticated, upload.single('image'), async (req: Request, res: Response): Promise<void> => {
  try {
    const petId = parseInt(req.params.id);
    const pet = await petService.getPetById(petId);
